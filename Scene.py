    ...
    def     drawCameraGrid(self):
        ...
        for p in camera_points:
            ...
            if s.get_collision(p) is None:
                ...
                self.canvas.create_oval(self.drawing_center[0] + p.x - 2, self.drawing_center[1] + p.y - 2,
                                        self.drawing_center[0] + p.x + 2, self.drawing_center[1] + p.y + 2)

        self.camera.direction = self.camera.direction.rotate(Vector(0.008, 0, 0))
